...
A dependency-free static site for the world's **1144 most famous recipes** — each
...
```
1144 recipes · 66 cuisines · 10 categories · 1282 static pages · 0 npm dependencies
```
...
│   │   ├── catalog-2.js …-17.js  # further volumes, same shape, merged at build
│   │   ├── details/*.js         # volume one long-form content
│   │   ├── details2/ …details17/ # long-form content for the matching volume
...
│       └── img/recipes/         # 2150 image files (WebP + JPEG)
├── tools/
...
├── 404.html
├── assets/                      #    css, js and 2150 image files
├── recipes/                     #    1144 recipe pages
...
- [x] **Recipe** on all 1144 recipe pages — `name`, `image`, `author`, `datePublished`, `prepTime`, `cookTime`, `totalTime`, `recipeYield`, `recipeCategory`, `recipeCuisine`, `keywords`, `nutrition`, `recipeIngredient`, `recipeInstructions` (as `HowToStep` with anchors), `suitableForDiet`
...
- [x] **FAQPage** on all 1144 recipe pages and the about page — 6,741 questions,
...
- [x] `node tools/keyword-audit.js` checks all 101,389 of them back against the
...
796 of the 1144 recipes have a photograph. Of the 1075 images on the site, 748
are CC0 or public domain, 147 are CC BY and 180 are CC BY-SA. Anything still
...
Every one of the site's 1144 recipes now passes, and `npm run check` runs the
...
The larger problem was waiting. **368 of the 1144 recipes** declare unattended
...
- **No JavaScript** — all 1144 recipes, navigation and taxonomy pages render fully from static HTML. Search, filtering, favourites and cook mode need JS.
