...
/* Generated pictures are listed, counted and described apart from the
...
for (const recipe of catalog) {
  ...
  for (const [kind, data] of [['Hero', entry.hero], ['Process', entry.process]]) {
    ...
    if (isIllustration(data)) {
      drawn++;
      drawnRows.push(
        `| \`${data.file}.webp\` / \`.jpg\` | ${recipe.title} | ${kind} | ` +
        `${(data.model || 'unknown model').replace(/\|/g, '/')} |`
      );
      continue;
    }
...
photographs, **${free}** are CC0 or public domain and carry no conditions at all, and
...
**${drawn}** of the pictures on this site are not photographs. They were drawn by
...
| Hero photographs | ${heroes} |
| Secondary "process" photographs | ${process_} |
| Generated illustrations | ${drawn} |
| Total image files | ${(heroes + process_ + drawn) * 2} (WebP + JPEG for each) |
...
console.log(`images-attribution.md written — ${rows.length} photographs, ${drawn} illustration(s), `
  + `${Object.keys(counts).length} licence types, ${missing.length} placeholder(s)`);
