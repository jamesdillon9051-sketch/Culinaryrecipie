...
/* Marked by the generator in src/data/images.json. Read from the record rather
   than guessed from the file, because nothing about a JPEG says how it was
   made. Kept in step with the same test in src/data/stats.js. */
const ILLUSTRATION = /^ai illustration$/i;
const isIllustration = img =>
  !!img && (ILLUSTRATION.test(img.source || '') || ILLUSTRATION.test(img.licence || ''));
...
function photoCredit(img, { compact = false } = {}) {
  ...
  /* A generated picture says so, in the place a photographer's name would go.
     This branch comes first and has no early return for a missing author,
     because an illustration has no author: without it the credit line would
     be empty and the drawing would sit on the page presenting itself as a
     photograph of the dish. tools/check.js fails the build if one of these
     ever renders without this notice. */
  if (isIllustration(img)) {
    return `<p class="photo-credit photo-credit--ai${compact ? ' photo-credit--compact' : ''}">`
      + `<span class="visually-hidden">Image note: </span>`
      + `Illustration, generated by AI &middot; not a photograph of this dish`
      + `</p>`;
  }
